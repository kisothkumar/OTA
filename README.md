...
## Bulk upload — .txt or .docx

On `teacher.html`, under **"Bulk upload questions"**, you can upload a
whole set of questions at once instead of adding them one by one.

1. Click **"Download template (.txt)"** to get the exact format expected.
2. Fill it in — one question per block, separated by a **blank line**:
   ```
   ...
   Time: 20
   ```
   Mark the correct option with a `*` right after it. `Time:` is optional
   (defaults to 20 seconds). Lines starting with `#` are ignored, so you
   can leave notes in the file.
3. Save as **.txt**, or as **.docx** from Word (File → Save As → Word
   Document). Click **"Choose file →"** and upload it.
4. Review the parsed preview — remove anything that came out wrong, then
   **"Add all to quiz"**. Optionally tick **"Also save to question bank"**
   to file the same questions under a subject for reuse later.

**A note on legacy `.doc` files:** the old binary Word format (`.doc`,
pre-2007) can't be parsed in a browser — there's no JavaScript library
that reads it reliably. If you upload one, the app will tell you to
re-save it as `.docx` or `.txt` from Word first (File → Save As). `.docx`
and `.txt` both work directly.
