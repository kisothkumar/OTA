...
// ======================================================================
// PASTE YOUR FIREBASE CONFIG BELOW — this is the ONLY place you need to
// edit to make the question bank (and live quizzes) sync across devices.
//
// How to get these values (free, ~3 minutes):
//   1. Go to https://console.firebase.google.com → "Add project" → name
//      it anything (e.g. "online-assessment") → finish the wizard.
//   2. In the left sidebar: Build → Realtime Database → "Create Database"
//      → pick a location → start in TEST MODE (you can lock it down
//      later, see the rules note in README.md).
//   3. In the left sidebar: Project settings (gear icon) → General →
//      scroll to "Your apps" → click the </> (web) icon → register the
//      app (nickname doesn't matter, skip hosting) → it will show you an
//      object exactly like FIREBASE_CONFIG below. Copy those real values
//      in below, replacing the YOUR_... placeholders.
//   4. Save this file. That's it — USE_FIREBASE below is already on, and
//      every page already loads the Firebase scripts it needs.
// ======================================================================
...
// Auto-detects whether you've pasted real values in above. Leave this as
// USE_FIREBASE_AUTO — no need to touch it.
const USE_FIREBASE_AUTO = !Object.values(FIREBASE_CONFIG).some(v => String(v).startsWith("YOUR_"));
const USE_FIREBASE = USE_FIREBASE_AUTO;
...
window.OA = (function () {
  ...
  if (USE_FIREBASE && !window.firebase) {
    console.warn("OA: USE_FIREBASE is on but the Firebase SDK didn't load (check your internet connection or the <script> tags in this page's <head>). Falling back to same-device sync.");
  }
  if (USE_FIREBASE && window.firebase) {
    try {
  ...
  const mode = FirebaseSync ? "firebase" : "local";
  if (mode === "local") {
    console.info("OA: running in same-device mode (localStorage). Paste your Firebase config into js/firebase-config.js to sync across devices — see the comment at the top of that file.");
  } else {
    console.info("OA: connected to Firebase — data syncs across devices.");
  }

  return { db, utils, effects, USE_FIREBASE, mode };
})();
