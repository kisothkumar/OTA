...
(function () {
  ...
  function addQuestionRow(prefill) {
    ...
    const correctIdx = typeof p.correct === "number" ? p.correct : 0;
    const time = p.time || 20;
    wrap.innerHTML = `
      <div class="row" style="justify-content:space-between;">
        <span class="small muted" style="text-transform:uppercase; letter-spacing:.08em;">Question ${idx + 1} ${p.fromBank ? '<span class="text-teal">· from bank</span>' : ""}</span>
        <button type="button" class="btn btn-ghost removeQ" style="padding:6px 14px; font-size:.8rem;">Remove</button>
      </div>
      <div class="field mt-16">
        <label>Question text</label>
        <input type="text" class="qText" placeholder="What powers the light-independent reactions?" value="${escapeAttr(p.text || "")}" />
      </div>
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">
        ${[0, 1, 2, 3].map(i => `
          <div class="field" style="margin-bottom:6px;">
            <label class="row gap-8">
              <input type="radio" name="correct-${idx}" class="qCorrect" value="${i}" ${i === correctIdx ? "checked" : ""} style="width:16px;height:16px;" />
              Option ${String.fromCharCode(65 + i)}
            </label>
            <input type="text" class="qOption" placeholder="Answer option ${String.fromCharCode(65 + i)}" value="${escapeAttr(opts[i] || "")}" />
          </div>
        `).join("")}
      </div>
      <div class="field" style="max-width:200px;">
        ...
        <select class="qTime">
          <option value="15" ${time == 15 ? "selected" : ""}>15 seconds</option>
          <option value="20" ${time == 20 ? "selected" : ""}>20 seconds</option>
          <option value="30" ${time == 30 ? "selected" : ""}>30 seconds</option>
          <option value="45" ${time == 45 ? "selected" : ""}>45 seconds</option>
          <option value="60" ${time == 60 ? "selected" : ""}>60 seconds</option>
        </select>
      </div>
    `;
  ...
  let bank = { subjects: [] };
  let activeBankSubject = null;

  const subjectSelect = document.getElementById("bankSubjectSelect");
  const bankQuestionList = document.getElementById("bankQuestionList");
  const bankEmpty = document.getElementById("bankEmpty");
  const bankSelectAll = document.getElementById("bankSelectAll");
  ...
  function renderSubjectOptions() {
    if (!bank.subjects.length) {
      subjectSelect.innerHTML = `<option value="">No subjects saved yet — visit Question Bank</option>`;
      renderBankQuestions();
      return;
    }
    subjectSelect.innerHTML = bank.subjects
      .map(s => `<option value="${s.id}">${s.name} (${s.questions.length})</option>`)
      .join("");
    activeBankSubject = bank.subjects[0].id;
    renderBankQuestions();
  }

  subjectSelect.addEventListener("change", () => {
    activeBankSubject = subjectSelect.value;
    renderBankQuestions();
  });

  function renderBankQuestions() {
    const subject = bank.subjects.find(s => s.id === activeBankSubject);
    const questions = subject ? subject.questions : [];
    bankSelectAll.checked = false;
    bankEmpty.classList.toggle("hidden", questions.length > 0);
    bankQuestionList.innerHTML = questions.map(q => `
      <label class="row gap-12" style="padding:10px 12px; background:var(--ink-900); border-radius:8px; cursor:pointer;">
        <input type="checkbox" class="bankQCheck" value="${q.id}" style="width:16px;height:16px;flex:0 0 auto;" />
  ...
  bankSelectAll.addEventListener("change", () => {
    bankQuestionList.querySelectorAll(".bankQCheck").forEach(cb => { cb.checked = bankSelectAll.checked; });
  });

  document.getElementById("bankAddSelectedBtn").addEventListener("click", () => {
    const subject = bank.subjects.find(s => s.id === activeBankSubject);
    if (!subject) return;
    const checked = [...bankQuestionList.querySelectorAll(".bankQCheck:checked")].map(cb => cb.value);
    if (checked.length === 0) {
      effects.toast("Tick at least one question first.");
      return;
    }
    // if the single starter row is still empty, replace it instead of leaving a blank row
    const firstRow = questionRows[0];
    if (questionRows.length === 1 && firstRow) {
      const firstText = firstRow.wrap.querySelector(".qText").value.trim();
      if (!firstText) {
        firstRow.wrap.remove();
        questionRows = [];
      }
    }
    checked.forEach(qid => {
      const q = subject.questions.find(x => x.id === qid);
      if (q) addQuestionRow({ text: q.text, options: q.options, correct: q.correct, time: q.time, fromBank: true });
    });
    renumber();
    effects.toast(`Added ${checked.length} question${checked.length > 1 ? "s" : ""} from the bank.`);
    bankQuestionList.querySelectorAll(".bankQCheck").forEach(cb => { cb.checked = false; });
    bankSelectAll.checked = false;
  });
